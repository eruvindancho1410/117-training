...
{
    private static bool UseMockedResponse => true;
    ...
    public async Task<string> GenerateJsonAsync(string input, string responseSchemaJson, CancellationToken cancellationToken = default)
    {
              ...
              "steps": [
                {
                  ...
                  "content": [
                    {
                      "type": "text",
                      "text": "{\"intent\":\"search\",\"status\":\"Cancelled\",\"memberTier\":\"Gold\",\"dateFrom\":\"2026-06-01\",\"dateTo\":\"2026-07-30\"}"
                    }
                  ]
                }
              ],
        ...
        if (UseMockedResponse)
        {
            _logger.LogInformation(
                "Gemini POST response: {HttpResponseCode} {ResponseMessage}",
                (int)HttpStatusCode.OK,
                mockedResponse);

            return ExtractModelOutput(mockedResponse);
        }
